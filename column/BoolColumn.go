...
func NewBoolColumn(name string) *BoolColumn {
	return &BoolColumn{datatype: "BOOL", name: name, allowNull: false, isUnique: false, alter: false}
}
...
func (col *BoolColumn) Alter() *BoolColumn {
	col.alter = true
	return col
}
...
func (col *BoolColumn) ToAlterSQL() string {
	if col.alter {
